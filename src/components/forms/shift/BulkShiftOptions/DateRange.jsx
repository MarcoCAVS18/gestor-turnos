...
const DateRange = ({
  ...
  return (
    <div className="space-y-4">
      {/* From Date */}
      <div>
        <FormLabel htmlFor="from-date">{t('forms.shift.bulk.startDate')}</FormLabel>
      ...
      {/* To Date */}
      <div>
        <FormLabel htmlFor="to-date">{t('forms.shift.bulk.endDate')}</FormLabel>
        <div className="flex items-center gap-2 mt-2">
          <Calendar size={16} style={{ color: colors.primary }} />
          <input
            id="to-date"
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-sm font-medium" style={{ color: colors.text }}>
                {t('forms.shift.bulk.skipWeekends')}
              </span>
            </div>
            <p className="text-xs" style={{ color: colors.textSecondary }}>
              {t('forms.shift.bulk.skipWeekendsDesc')}
            </p>
          </div>
          <Switch
            id="skip-weekends"
        ...
        >
          ...
          <div>
            <p className="text-sm font-medium" style={{ color: colors.text }}>
              {t('forms.shift.bulk.shiftsWillBeCreated', { count: dayCount })}
            </p>
            <p className="text-xs mt-1" style={{ color: colors.textSecondary }}>
              {t('forms.shift.bulk.fromToDate', { from: formatDate(fromDate), to: formatDate(toDate) })}
              {skipWeekends && ` (${t('forms.shift.bulk.weekdaysOnly')})`}
            </p>
          </div>
        </div>
      ) : (
        fromDate && toDate && (
          ...
          >
            ...
            <p className="text-sm" style={{ color: colors.error }}>
              {t('forms.shift.bulk.endDateAfterStart')}
            </p>
          </div>
        )
      )}
