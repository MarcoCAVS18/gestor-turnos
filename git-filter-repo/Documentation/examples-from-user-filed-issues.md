...
```
git ls-tree -r $***REMOVED***BRANCH1***REMOVED*** >../my-files
git ls-tree -r $***REMOVED***BRANCH2***REMOVED*** >>../my-files
...
history periodically.  First, identify an $***REMOVED***OLD_COMMIT***REMOVED*** that you want
to be a new root commit, then run:

```
git replace --graft $***REMOVED***OLD_COMMIT***REMOVED***
git filter-repo --proceed
```

(The trick here is that `git replace --graft` takes a commit to replace, and
a list of new parents for the commit.  Since $***REMOVED***OLD_COMMIT***REMOVED*** is the final
...
```
git log -1 --raw --no-abbrev $***REMOVED***COMMIT_WHERE_YOU_COMPRESSED_PNGS***REMOVED***
```
