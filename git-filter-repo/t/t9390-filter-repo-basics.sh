...
filter_testcase() ***REMOVED***
	...
	NAME="check: $INPUT -> $OUTPUT using '$***REMOVED***REST[@]***REMOVED***'"
	test_expect_success "$NAME" '
		...
		# Run the example
		cat $DATA/$INPUT | git filter-repo --stdin --quiet --force --replace-refs delete-no-add "$***REMOVED***REST[@]***REMOVED***" &&
...
create_path_filtering_and_renaming() ***REMOVED***
	...
	(
		...
		>.gitignore &&
		mkdir -p src/main/java/com/org/***REMOVED***foo,bar***REMOVED*** &&
...
setup_commit_message_rewriting() ***REMOVED***
	...
	(
		...
		echo hello >world &&
		git add world &&
		git commit -m "Commit referencing $***REMOVED***name:0:8***REMOVED***" &&
...
test_expect_success 'commit message rewrite' '
	...
	(
		...
		echo "Commit referencing $***REMOVED***name:0:8***REMOVED***" >expect &&
		...
		latest=$(git log --no-walk | grep reverts | awk "***REMOVED***print \$4***REMOVED***" | tr -d '.') &&
		test -n "$latest" &&
...
setup_handle_funny_characters() ***REMOVED***
...
test_expect_success '--state-branch with changing renames' '
	...
	(
		...
		cat <<-EOF >expect &&
			Merge branch $***REMOVED***SQ***REMOVED***A$***REMOVED***SQ***REMOVED*** into B
			add twenty
